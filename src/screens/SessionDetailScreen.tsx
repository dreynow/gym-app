  ...
  if (session === null) {
    return (
      <>
        ...
        <p className="p-6 text-muted">This workout no longer exists.</p>
      </>
    )
  }
  ...
  return (
    <>
      ...
      <div className="p-4 space-y-4">
        ...
        {(prs?.length ?? 0) > 0 && (
          <Card className="p-4">
            <div className="flex items-center gap-2 text-pr font-semibold mb-2">
            ...
            return (
              <Card key={i} className="p-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {entryPRs.length > 0 && (
                    <IconTrophy size={16} className="text-pr shrink-0" />
                  )}
                </div>
                  ...
                  {entry.sets.filter(isLogged).map((set, j) => (
                    <div key={j} className="flex items-center gap-3 text-sm">
                      <span
                        className={cx(
                          'w-14 text-xs',
                          set.type === 'warmup' ? 'text-warn' : 'text-faint',
                        )}
                      >
                      ...
                      {set.rpe != null && (
                        <span className="text-faint text-xs">RPE {num(set.rpe)}</span>
                      )}
                    </div>
                  ))}
                </div>
